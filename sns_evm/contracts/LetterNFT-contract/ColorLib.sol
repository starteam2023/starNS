...
library ColorLib {
    ...
    /// Lookup table for cubicinout range 0-99 立方输入
    ...
    function getLetterForAddress(address addr,string memory _letters)
        internal
        pure
        returns (string memory)
    {
            ...
            uint8 num=uint8(lerpHue(
                        uint8(addrBytes[31 - 3]),
                        uint8(addrBytes[31 - 6]) % 2,
                        startHue,
                        0
                    )%bytes(_letters).length);
            return string(abi.encodePacked(bytes(_letters)[num]));
    }
}